...
def get_request_time(src, dst):
    get_pod = shlex.split("kubectl --context {} exec -it {} -c {} -- curl -w \"{}\" -s -o /dev/null --user {}:password -X POST --header 'Content-Type: application/json' --header 'Accept: text/html' -d '{{ \"document\": \"Contents of the document\", \"document_name\": \"file_name_to_save\" }}' http://{}:{}/api/{} -v".format(src.context, src.pod_id, src.name, src.name, args.curl_format, dst.service_ip, dst.service_port, dst.name))
    if args.verbose >= 3:
